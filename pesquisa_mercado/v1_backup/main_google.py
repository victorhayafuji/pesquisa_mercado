...
def main() -> None:
    ...
    # Timestamp centralizado da execução
    agora = datetime.now()

    # Coluna de timestamp da captura
    df["capturado_em"] = agora.strftime("%Y-%m-%d %H:%M:%S")
    ...
    # =========================
    data_exec = agora.strftime("%Y-%m-%d")
    ...
    print(f"Arquivo CSV (padronizado p/ análise): {out_csv_analise}")

    # Exportar também para XLSX
    out_xlsx_analise = ANALISE_DIR / "xlsx" / data_exec / f"Analise_GoogleShopping_{slug}_{timestamp}.xlsx"
    if _exportar_xlsx_ptbr(df_analise, out_xlsx_analise):
        print(f"Arquivo Excel (padronizado p/ análise): {out_xlsx_analise}")
